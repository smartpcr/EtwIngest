...
```bash
./bin/Debug/net6.0/EtwEventReader trace.etl
```
...
1. **Read a single ETL file with summary output:**
   ```bash
   EtwEventReader trace.etl
   ```

2. **Read all ETL files in a directory with detailed output:**
   ```bash
   EtwEventReader -f detailed C:\Logs\
   ```

3. **Filter by provider name:**
   ```bash
   EtwEventReader -p "Microsoft-Windows-Kernel-Process" trace.etl
   ```

4. **Filter by event name with tree output:**
   ```bash
   EtwEventReader -e "ProcessStart" -f tree *.etl
   ```

5. **Filter by activity ID:**
   ```bash
   EtwEventReader -a "12345678-1234-1234-1234-123456789012" trace.etl
   ```

6. **Show detailed error information:**
   ```bash
   EtwEventReader --errors trace.etl
   ```

## Command Line Options

```
Usage: EtwEventReader [options] <path1> [path2] ...
...
```
EtwEventReader/
├── EtwEventReader.csproj          # Project file
